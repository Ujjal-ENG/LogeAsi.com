...
function UpdateProduct() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.patch(`http://localhost:8080/api/v1/product/update-product/${state._id}`, product, {
                headers: {
                    // eslint-disable-next-line comma-dangle
                    Authorization: userInfo?.token
                }
            ...
            console.log(error);
            toast.error('Error ocurred when Updating the Product!!');
        }
    };
    return (
        <div>
            <h1 className="text-3xl font-bold text-center py-4">Update Product Here</h1>
